...
export default function MockModule({ initialCaseType }: MockModuleProps) {
  const [caseType, setCaseType] = useState<CaseType | null>(
    (initialCaseType as CaseType) || null
  );
  const [messages, setMessages] = useState<Message[]>(() => {
    if (initialCaseType && CASE_PROMPTS[initialCaseType as CaseType]) {
      return [{ role: "ai", text: CASE_PROMPTS[initialCaseType as CaseType] }];
    }
    return [];
  });
